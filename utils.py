...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    for i in range(0, len(data), batch_size):
      batched_data.append(np.asarray(data[i:i+batch_size]))
      batched_label.append(np.asarray(label[i:i+batch_size]))
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.plot(x, train_loss_history, 'g')
    plt.plot(x, valid_loss_history, 'r')
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.plot(x, train_acc_history, 'g')
    plt.plot(x, valid_acc_history, 'r')
    plt.legend(['train', 'valid'])
    plt.show()
